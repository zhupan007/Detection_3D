...
class Suncg():
  ...
  def __init__(self, root_path):
    ...
    house_fns.sort()
    if SAGE:
      self.house_fns = house_fns[1700: 2000]
    else:
      self.house_fns = house_fns[1500: 1700]
...
def gen_pcl(house_fn):
    ...
    pre_downsample_num = int(max_point_num / cam_num)
...
def gen_cam_images(house_fn):
    ...
    min_visible_objects = 1 # 3
    angle_sampling = np.pi/6.0 # np.pi / 3.0
    ...
    if gen_new_cam or always_add_extra_cam:
